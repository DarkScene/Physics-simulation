...
{
	static cyclone::Random randn;

	for (int i = 0; i < m_moverconnection->m_movers.size(); i++) {
		m_moverconnection->m_movers[i]->addForceAtBodyPoint(cyclone::Vector3(0, 10000, 0), cyclone::Vector3(randn.randomReal(), randn.randomReal(), randn.randomReal()));
	}
}
