...
unsigned ParticleCollision::addContact(cyclone::ParticleContact *contact, unsigned limit) const {
	...
	double collisionLength = (particle[0]->getPosition() - particle[1]->getPosition()).magnitude();

	if (collisionLength < size * 2) {
		contact->particle[0] = particle[0];
		contact->particle[1] = particle[1];
		contact->contactNormal = (particle[0]->getVelocity() - particle[1]->getVelocity()).unit() * -1;
		contact->penetration = (size * 2) - collisionLength;
		contact->restitution = 1.0;
