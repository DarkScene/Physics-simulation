...
//==========================================================================
{
	...
	c->init(&m_mover.m_particle, 2.0);
	m_contactGenerators.push_back(c);
	m_resolver = new cyclone::ParticleContactResolver(2);
}
...
void MyGlWindow::update()
{
	...
	unsigned limit = 1;  //이 경우에는 파티클 한 개와 바닥 뿐이므로 1로 설정
...
//==========================================================================
{
  ...
  case FL_DRAG: // if the user drags the mouse
  {
	  if (selected >= 0 && m_pressedMouseButton == 1) {
		  ...
		  double rx, ry, rz;
		  mousePoleGo(r1x, r1y, r1z, r2x, r2y, r2z, static_cast<double>(m_mover.m_particle.getPosition().x), static_cast<double>(m_mover.m_particle.getPosition().y), static_cast<double>(m_mover.m_particle.getPosition().z), rx, ry, rz, (Fl::event_state() & FL_CTRL) != 0);

		  m_mover.m_particle.setPosition(rx, ry, rz);
