...
//Draw axises
  ...
  glColor3f(1.0f, 1.0f, 0);  //선 색상
  glLineWidth(5.0f);  //선 굵기
  glPushMatrix();
  glBegin(GL_LINES);
  glVertex3f(5, 15, 5);  //시작점
  glVertex3f(5, 0, 5); //끝점
  glEnd();
  glPopMatrix();
}
...
//==========================================================================
{
  ...
  case FL_DRAG: // if the user drags the mouse
  {
	  if (selected >= 0 && m_pressedMouseButton == 1) {
		  ...
		  double rx, ry, rz;
		  mousePoleGo(r1x, r1y, r1z, r2x, r2y, r2z, static_cast<double>(m_connect.m_movers[selected]->m_particle.getPosition().x), static_cast<double>(m_connect.m_movers[selected]->m_particle.getPosition().y), static_cast<double>(m_connect.m_movers[selected]->m_particle.getPosition().z), rx, ry, rz, (Fl::event_state() & FL_CTRL) != 0);

		  m_connect.m_movers[selected]->m_particle.setPosition(rx, ry, rz);
